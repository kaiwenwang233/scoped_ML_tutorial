...
c
c  f77 -O -o pairs pairs.f indexmap.f ifindi.f indexxi.f freeunit.f

	implicit none

	integer MAXEVE, MAXPAIR
	parameter(MAXEVE=300000,MAXPAIR=106725463)
	character*100 line 
	character*100 fn_ids
...
c	real hdist,vdist
	...
	integer data(MAXEVE+2+2*MAXPAIR)
	character str*855003712
	equivalence(data,str)
...
c --- Read input catalog
	write(*,*) "Reading in catalog file..."
        call freeunit(iunit)
      	open(iunit,file='merg.cat',status='unknown')
...
c --- Select events within 200 km of station
c	ct=0
c	slat=37.206833   
c	slon=-121.797333
c	stadist=150.
...
c	   print *, lat(i), lon(i), slat, slon, del, dist, az, i
	...
	data(nev+2)=999
     ...
     &                     (dep(i)-dep(j))**2 )
              if(dist.le.maxdist) then
                  npair=npair+1  
	          data(2*npair+nev+1)=i
	   	  data(2*npair+nev+2)=j
              endif
           enddo
        enddo
...
c --- Output to binary file
	size=4*(nev+2+2*npair)
	write(*,*) 'Writing out binary file (bytes): ', size
	call freeunit(iunit)
        open(iunit,file='pair.out',form='unformatted',access='direct',
     &          recl=size)
	write(iunit,rec=1) str(1:size)
	close(iunit)
