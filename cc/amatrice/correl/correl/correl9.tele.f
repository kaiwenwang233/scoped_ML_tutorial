...
c --- Input parameters
	...
	t2=8.			! seconds after pick to read in, window is t1+t2
	w1=7.			! window length of short window (seconds)
	delta=0.05		! sample interval, resamples if necessary
c	delta=0.02		! sample interval, resamples if necessary
	icut='P'		! phase to read in
c	icut='S'
	lag=3.			! lags to search over (seconds)
	iswap=.false.		! swap bytes flag
c	iswap=.true.
	ifilter=.true.		! filter flag	
	lowcorner=0.5		! low pass corner (Hz)
	highcorner=2.		! high pass corner (Hz)
	maxdist=100.		! maximum distance to correlate (km)
	ccthr=0.4		! CC threshold to save data
...
c	i=system('date')
	...
	i=system('date')
        call readsac(nevsub,files,dir,t1,t2,delta,icut,iswap,lunit,
     &		     data,nev2,nsamp,tref,ind)

c        call readmseed(nevsub,files,dir,t1,t2,delta,icut,iswap,
c     &		     lunit,data,nev2,nsamp,tref,ind)
...
c	print *, nsamp, nlag, isub, jsub

c	if (jsub.eq.54) then

c	do n=1,nsamp
c      	   print *, y2(n)
c	   if (isnan(y2(n))) then
c		print *, "yes"
c	   endif
c      	enddo
...
c	print *, nsamp-nw2+nw1, nlag, nw2, nw1
