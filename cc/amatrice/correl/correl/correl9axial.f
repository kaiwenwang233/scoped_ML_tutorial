...
c --- Input parameters
	...
	t2=0.75		! seconds after P-pick to read in, window is t1+t2
	w1=0.5		! P-window length of short window (seconds)
	t2=1.0		! seconds after S-pick to read in, window is t1+t2
	w1=0.75		! S-window length of short window (seconds)
	delta=0.005		! sample interval, resamples if necessary
c	icut='P'		! phase to read in
        icut='S'
	lag=0.5			! lags to search over (seconds)
c	iswap=.false.		! swap bytes flag
	iswap=.true.
	ifilter=.true.		! filter flag	
	lowcorner=4.		! low pass corner (Hz)
	highcorner=50.		! high pass corner (Hz)
	maxdist=5.		! maximum distance to correlate (km)
	maxdist=2.		! maximum distance to correlate (km)
	ccthr=0.7		! CC threshold to save data
	maxdist=1.		! maximum distance to correlate (km)
	ccthr=0.8		! CC threshold to save data
