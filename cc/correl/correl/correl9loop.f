...
c --- works looping through stations if adjust output header correctly.
      ...
      character*100 fn_log, fn_out
      character*100 fn_ids, fn_cat
      character*100 fn_newids(MAXSTA)
      character*30 prodir
...
12       read (iunit,'(a)',end=13) line
      	...
      	if (l.eq.1) read (line,*,err=893) t1
      	if (l.eq.2) read (line,*,err=893) t2
      	if (l.eq.3) read (line,*,err=893) w1
      	if (l.eq.4) read (line,*,err=893) delta
      	if (l.eq.5) read (line,*,err=893) lag
      	if (l.eq.6) read (line,*,err=893) iiswap
      	if (l.eq.7) read (line,*,err=893) iifilter
      	if (l.eq.8) read (line,*,err=893) lowcorner
      	if (l.eq.9) read (line,*,err=893) highcorner
      	if (l.eq.10) read (line,*,err=893) ccthr
...
c	read(*,*) fn_pair
	print *, "enter old catalog file:"
	read(*,*) fn_catold
	print *, "enter new catalog file:"
	read(*,*) fn_catnew
...
c	iswap=.false.		! swap bytes flag
	...
	maxdist=5.		! maximum distance to correlate (km)
...
c	write(*,*) "Reading in old catalog file..."
c	i=system('date')
c        call freeunit(iunit)
c      	open(iunit,file=fn_catold,status='unknown')
...
c     &                mag,ids(i)
c	ind1(i)=0
c	if(i.lt.5) print *, yr,mo,day,hr,min,ids(i)
c	i=i+1
c	goto 10
c20	nev=i-1
c	close(iunit)
...
c	i=system('date')
	...
	i=1
15	read(iunit,*,end=25) yr,mo,day,hr,min,sec,lat(i),lon(i),dep(i),
...
c	i=system('date')
	...
	call freeunit(iunit)
	open(iunit,file='cids',status='unknown')
	do i=1,nev
          write(iunit,*) ids(i)
	enddo
	close(iunit)
...
60	nsta=i-1
	...
	do l=1,2	! loop over P and S phases
...
c --- write ids header to database file
      ...
      fn_cc=fn_out(1:trimlen(fn_out)) // '/corr // icut(l:l)
     &	// '/' // sta(i3)(1:trimlen(sta(i3))) // '.cc'
      call freeunit(iunit)
...
c maybe need to intialize idssub?  no nevsub should take care of.
...
c     &		     data,nev2,nsamp,tref,ind)
        call readmseed(nevsub,files,dir,t1,t2,delta,icut(l:l),iswap,
...
100     continue
	...
	enddo  ! loop over P and S phases
