...
c --- works looping through stations if adjust output header correctly.
      ...
      character*100 fn_ids, fn_catold, fn_catnew
      character*30 prodir
...
c	i=system('date')
        call freeunit(iunit)
      	open(iunit,file=fn_catold,status='unknown')
...
c --- Read input new catalog
	write(*,*) "Reading in new catalog file..."
c	i=system('date')
        call freeunit(iunit)
      	open(iunit,file=fn_catnew,status='unknown')

15	read(iunit,*,end=25) yr,mo,day,hr,min,sec,lat(i),lon(i),dep(i),
     &                mag,ids(i)
	ind1(i)=1
	if(i.lt.iold+5) print *, yr,mo,day,hr,min,ids(i)
	i=i+1
	goto 15
25	nev=i-1
	close(iunit)
