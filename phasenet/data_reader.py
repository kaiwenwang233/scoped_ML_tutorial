...
class DataReader:
    ...
    def read_mseed_array(self, fname, stations, amplitude=False, remove_resp=True):
        ...
        for i in range(len(mseed)):
            if mseed[i].stats.sampling_rate != self.config.sampling_rate:
                ...
                try:
                    ...
                    logging.warning(f"Resampling {mseed[i].id} failed")
        mseed = mseed.select(sampling_rate=self.config.sampling_rate)
        try:
            mseed = mseed.detrend("spline", order=2, dspline=5 * mseed[0].stats.sampling_rate)
        except:
        ...
        mseed = mseed.merge(fill_value=0)
        mseed = mseed.filter("highpass", freq=4.0)
        starttime = min([st.stats.starttime for st in mseed])
        endtime = max([st.stats.endtime for st in mseed])
        mseed = mseed.trim(starttime, endtime, pad=True, fill_value=0)
        ...
        for i in range(nsta):
            ...
            if not empty_station:
                ...
                data.append(np.pad(trace_data[1500:,:],((0,1500),(0,0))))
                # data.append(np.pad(trace_data[1200:,:],((0,1200),(0,0))))
                # data.append(np.pad(trace_data[1800:,:],((0,1800),(0,0))))
                # data.append(np.pad(trace_data[2400:,:],((0,2400),(0,0))))
                if amplitude:
                    raw_amp.append(trace_amp)
                    raw_amp.append(np.pad(trace_amp[1500:,:],((0,1500),(0,0))))
                    # raw_amp.append(np.pad(trace_amp[1200:,:],((0,1200),(0,0))))
                    # raw_amp.append(np.pad(trace_amp[1800:,:],((0,1800),(0,0))))
                    # raw_amp.append(np.pad(trace_amp[2400:,:],((0,2400),(0,0))))
                ...
                t0.append((starttime+15).strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3])
                # t0.append((starttime+6).strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3])
                # t0.append((starttime+9).strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3])
                # t0.append((starttime+12).strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3])
        if len(data)>0:
            data = np.stack(data)
            if len(data.shape) == 3:
                data = data[:, :, np.newaxis, :]
