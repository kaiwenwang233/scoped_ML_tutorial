...
def get_prediction(data, return_preds=False):
    ...
    amps_all=[]
    preds_all=[]
    for i in range((nt-window)/shift+1):

        vec_slidingwindow=vec[:,i*shift:i*shift+window,:,:]


	feed = {model.X: vec_slidingwindow, model.drop_rate: 0, model.is_training: False}
	...
	picks_all.append(picks.tolist())
        amps_all.append(amps.tolist())

    #picks = format_picks(picks, data.dt, amps)
    picks = format_picks(picks_all, data.dt, amps_all)


    if return_preds:
        return picks, preds_all
    return picks
